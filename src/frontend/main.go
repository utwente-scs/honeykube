...
type frontendServer struct {
	...
	userDbSvcAddr string
	userDbSvcConn *grpc.ClientConn
}
...
func main() {
	...
	mustMapEnv(&svc.userDbSvcAddr, "USER_DB_SERVICE_ADDR")
	...
	mustConnGRPC(ctx, &svc.userDbSvcConn, svc.userDbSvcAddr)

	r := mux.NewRouter()
	r.HandleFunc("/", svc.homeHandler).Methods(http.MethodGet, http.MethodHead)
	r.HandleFunc("/product/{id}", svc.productHandler).Methods(http.MethodGet, http.MethodHead)
	r.HandleFunc("/cart", svc.viewCartHandler).Methods(http.MethodGet, http.MethodHead)
	r.HandleFunc("/user", svc.userHandler).Methods(http.MethodGet, http.MethodHead)
	r.HandleFunc("/user/login", svc.userLoginHandler).Methods(http.MethodPost)
	r.HandleFunc("/user/signup", svc.userSignUpHandler).Methods(http.MethodPost)
	...
	err := httpscerts.Check("/tmp/cert.pem", "/tmp/key.pem")
	// If they are not available, generate new ones.
	if err != nil {
		err = httpscerts.Generate("/tmp/cert.pem", "/tmp/key.pem", addr+":"+srvPort)
		if err != nil {
			log.Fatal("Error: Couldn't create https certs.")
		}
	}
	err1 := http.ListenAndServeTLS(addr+":"+srvPort, "/tmp/cert.pem", "/tmp/key.pem", handler)
	if err1 != nil {
		log.Fatal(err1)
	}
}
